...
func (ball *ball) update(leftPaddle *paddle, rightPaddle *paddle) {
	...
	if int(ball.x) < int(leftPaddle.x)+leftPaddle.w/2 {
		if int(ball.y) > int(leftPaddle.y)-int(leftPaddle.h/2) &&
			int(ball.y) < int(leftPaddle.y)+int(leftPaddle.h/2) {
			ball.xv = -ball.xv
		}
	}
	if int(ball.x) > int(rightPaddle.x)-rightPaddle.w/2 {
		if int(ball.y) > int(rightPaddle.y)-int(rightPaddle.h/2) &&
			int(ball.y) < int(rightPaddle.y)+int(rightPaddle.h/2) {
			ball.xv = -ball.xv
		}
	}
}
